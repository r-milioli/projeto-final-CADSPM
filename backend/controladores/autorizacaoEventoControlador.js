    ...
    const [r] = await pool.query(
      `INSERT INTO autorizacoes_eventos
       (usuario_id, protocolo, titulo, descricao, tipo_evento, endereco, bairro, cidade, estado, cep,
        data_inicio, data_fim, publico_estimado, observacoes)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [usuario_id, protocolo, titulo, descricao || null, tipo_evento || null,
       endereco, bairro, cidade, estado, cep || null,
       data_inicio, data_fim, publico_estimado || null, observacoes || null]
    );
    return res.status(201).json({ id: r.insertId, protocolo, status: 'RECEBIDA' });
  } catch (e) {
    console.error(e);
...
export async function obterPorProtocolo(req, res) {
  try {
    ...
    const [rows] = await pool.query('SELECT * FROM autorizacoes_eventos WHERE protocolo = ? LIMIT 1', [protocolo]);
    if (!rows.length) return res.status(404).json({ erro: 'Protocolo não encontrado' });
    const reg = rows[0];
    if (req.usuario.tipo_usuario !== 'ADMIN' && reg.usuario_id !== req.usuario.id) {
...
export async function editarAutorizacaoEvento(req, res) {
  try {
    ...
    if (!rows.length) return res.status(404).json({ erro: 'Protocolo não encontrado' });
    const reg = rows[0];
    if (reg.status !== 'RECEBIDA') return res.status(400).json({ erro: 'Edição não permitida neste status' });
    if (reg.usuario_id !== req.usuario.id) return res.status(403).json({ erro: 'Sem permissão' });
    ...
    await pool.query(
      `UPDATE autorizacoes_eventos SET
         ...
         data_inicio=?, data_fim=?, publico_estimado=?, observacoes=?
       WHERE protocolo=?`,
      [titulo, descricao || null, tipo_evento || null, endereco, bairro, cidade, estado, cep || null,
       data_inicio, data_fim, publico_estimado || null, observacoes || null, protocolo]
    );
    const [atualizado] = await pool.query('SELECT atualizado_em FROM autorizacoes_eventos WHERE protocolo=?', [protocolo]);
...
export async function cancelarAutorizacaoEvento(req, res) {
  try {
    const { protocolo } = req.params;
    const [rows] = await pool.query('SELECT usuario_id, status FROM autorizacoes_eventos WHERE protocolo = ? LIMIT 1', [protocolo]);
    if (!rows.length) return res.status(404).json({ erro: 'Protocolo não encontrado' });
    const reg = rows[0];
    if (reg.status !== 'RECEBIDA') return res.status(400).json({ erro: 'Cancelamento não permitido neste status' });
    if (reg.usuario_id !== req.usuario.id) return res.status(403).json({ erro: 'Sem permissão' });

    await pool.query('UPDATE autorizacoes_eventos SET status="CANCELADA" WHERE protocolo=?', [protocolo]);
...
async function buscarAEPorProtocolo(protocolo) {
  const [r] = await pool.query('SELECT * FROM autorizacoes_eventos WHERE protocolo=? LIMIT 1', [protocolo]);
...
export async function listarAnexos(req, res) {
  try {
    ...
    if (!isAdmin(req) && !isOwner(req, ae)) return res.status(403).json({ erro: 'Sem permissão' });
...
export async function anexarArquivoGravado(req, res) {
  try {
    ...
    if (!isAdmin(req) && (!isOwner(req, ae) || ae.status !== 'RECEBIDA')) {
      return res.status(403).json({ erro: 'Não é permitido anexar neste status' });
    }

    if (!req.file) return res.status(400).json({ erro: 'Arquivo ausente' });
    const { originalname, size, path: savedPath, mimetype, filename } = req.file;
...
export async function baixarAnexo(req, res) {
  try {
    ...
    if (!isAdmin(req) && !isOwner(req, row)) return res.status(403).json({ erro: 'Sem permissão' });

    res.setHeader('Content-Type', row.mime || 'application/octet-stream');
    res.setHeader('Content-Disposition', `attachment; filename="${slug(row.nome_original)}"`);
...
export async function excluirAnexo(req, res) {
  try {
    ...
    if (!isAdmin(req)) {
      if (!isOwner(req, row)) return res.status(403).json({ erro: 'Sem permissão' });
      if (row.status !== 'RECEBIDA') return res.status(400).json({ erro: 'Não é possível excluir neste status' });
    }

    // apaga arquivo físico (ignora erro se não existir)
    try { await fs.rm(row.caminho_ou_chave, { force: true }); } catch {}
